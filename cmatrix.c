...
static int read_value_from_file(const char* path)
{
  FILE* f = fopen(path, "r");
  if (!f)
    return -1;
      ...
      if (msg[0] != '\0')
        {
          ...
          snprintf(counter_buf, sizeof(counter_buf),
                   "  Temps écoulé : %02d:%02d", hours, minutes);
          // char pwd_buf[128];
          // snprintf(pwd_buf, sizeof(pwd_buf), "Test: %d",
          //          read_value_from_file("/tmp/customLock.out"));

          int base_y = msg_x + total_lines / 2 + 2;
          int base_x = COLS / 2;

          move(base_y, base_x - strlen(counter_buf) / 2);
          addstr(counter_buf);

          // move(base_y + 1, base_x - strlen(pwd_buf) / 2);
          // addstr(pwd_buf);
        }
