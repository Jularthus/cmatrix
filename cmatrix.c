...
void usage(void) {
  printf(" Usage: cmatrix -[abBcfhlsmVxk] [-u delay] [-C color] [-t tty] [-M "
         "message]\n");
  printf(" -a: Asynchronous scroll\n");
  printf(" -b: Bold characters on\n");
  printf(" -B: All bold characters (overrides -b)\n");
  printf(" -c: Use Japanese characters as seen in the original matrix. "
...
int main(int argc, char *argv[]) {
  ...
  if (console) {
    ...
    if (va_system("consolechars -f matrix") != 0) {
      c_die(" There was an error running consolechars. Please make sure the\n"
  ...
  while (1) {
    ...
    if (msg[0] != '\0') {
      ...
      // Dupliquer le message pour pouvoir le modifier
      char *msg_copy = strdup(msg);
      char *line = strtok(msg_copy, "\n");

      // Compter le nombre de lignes
      int total_lines = 0;
      for (char *tmp = strdup(msg); tmp && strtok(tmp, "\n"); tmp = NULL)
        total_lines++;

      int line_index = 0;
      while (line != NULL) {
        int msg_y = COLS / 2 - strlen(line) / 2;
        int y = (msg_x - total_lines / 2 + line_index) - 1;

        // Bordure au-dessus de la première ligne
        // if (line_index == 0) {
        //   move(y - 1, msg_y - 2);
        ...
        // Bordure en dessous de la dernière ligne
      ...
      // === Ajout du compteur sous le message ===
      ...
      char counter_buf[128];
      snprintf(counter_buf, sizeof(counter_buf), "  Temps écoulé : %02d:%02d",
               hours, minutes);

      move(msg_x + total_lines / 2 + 2, COLS / 2 - strlen(counter_buf) / 2);
      addstr(counter_buf);
    }
